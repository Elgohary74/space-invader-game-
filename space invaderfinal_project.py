...
pygame.display.set_caption("Space Shooter Game_ZC")
...
class Laser:
    ...
    def move(self, vel):
        self.y += vel #laser moves vertical
...
class Player(Ship):
    def __init__(self, x, y, health=100):
        super().__init__(x, y, health) #retuen objects of the suber class
        self.ship_img = YELLOW_SPACE_SHIP
        self.laser_img = YELLOW_LASER
        self.mask = pygame.mask.from_surface(self.ship_img)
        self.max_health = health #to evalute health bar
...
def main():
    ...
    enemies = [] #control enemies
    wave_length = 5 #num of enemies in each level
    enemy_vel = 1
    ...
    while run:
        ...
        if lost:
            if Lost_count > FPS * 3: #3 second passed after lost
        ...
        if len(enemies) == 0:
            ...
            for i in range(wave_length):
                enemy = Enemy(random.randrange( #select randomly the position and the color of the enmy
                    50, WIDTH-100), random.randrange(-1300, -100), random.choice(["red", "blue", "green"]))
                enemies.append(enemy)

        # Player Movement
        keys = pygame.key.get_pressed()
        if keys[pygame.K_a] and player.x - player_vel > 0:  # left and not get out left screen
            player.x -= player_vel
        ...
        if keys[pygame.K_SPACE]: # for shooting:
            player.shoot()
        ...
        for enemy in enemies:
            ...
            if random.randrange(0, 2*60) == 1: #shoot each 2 sconds
                enemy.shoot()

            if collide(enemy, player): #if collision -10 health and remove enemy
                player.health -= 10
